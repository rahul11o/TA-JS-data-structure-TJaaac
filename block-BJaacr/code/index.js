...
// You have access to a variable named `got`. Which comes from `d.js` file
...
// 4. Log the name of all the people in house Starks (index 0) one by one
 for(let pplName of got.houses[0].people){
   console.log(pplName.name)
 }

// 5. Log the name of all the people in house Lannisters (index 1) one by one
 for (let pplName of got.houses[1].people){
   console.log(pplName.name)
}
...
//    let description = got.houses[1].people[i].description;
//    let name = got.houses[1].people[i].name;
//    console.log(`I am ${name} and my bio is ${description}`)
 }
 */

for(let nameAndDesc of got.houses[1].people){
  console.log(`I am ${nameAndDesc.name}  and my bio is ${nameAndDesc.description}`)
}
...
// 14. Add the name and size of people in an object like {Starks: 4, Lannisters: 6} and log the object
   let memberInfo = {};
   for (let nameAndSize of got.houses){
    memberInfo[nameAndSize.name] = nameAndSize.people.length
   }
   console.log(memberInfo);
// 15. Log the name of all the people of all the houses in got one by one
for (let ppl of got.houses){
  for(let names of ppl.people){
